...
def print_banner(config: AgentConfig, skills_count: int, memory_enabled: bool) -> None:
    memory_status = "[green]✓ enabled[/green]" if memory_enabled else "[dim]disabled[/dim]"
    content = (
        f"[bold cyan]File Agent[/bold cyan]  [dim]v{config.agent.version}[/dim]\n"
        f"[dim]Model:[/dim] [white]{config.llm.model}[/white]  [dim]·[/dim]  [dim]Ollama Cloud[/dim]\n"
        f"[dim]Skills:[/dim] [white]{skills_count}[/white]  [dim]·[/dim]  [dim]Memory:[/dim] {memory_status}\n\n"
...
def print_response(content: str, trace: TurnTrace | None) -> None:
    ...
    _console.print()
    _console.print(Panel(rendered, border_style=border, title="[bold]Agent[/bold]", padding=(1, 2)))
...
def print_help() -> None:
    ...
    commands = [
        ...
        ("/clear", "Clear context history and screen"),
        ("/help", "Show this help"),
        ("/traces", "Last 3 traces of the day"),
        ("/traces N", "Last N traces"),
        ("/stats", "Current session statistics"),
        ("/memory", "List saved memory facts"),
        ("/memory buscar <query>", "Search memory"),
        ("/memory clear", "Delete all facts"),
        ("/model", "Active model and LLM config"),
    ]
    for cmd, desc in commands:
        table.add_row(cmd, desc)

    _console.print(Panel(table, title="[bold]Help[/bold]", border_style="blue", padding=(1, 1)))
...
def print_trace(trace: TurnTrace, verbose: bool = False) -> None:
    ...
    errors = " [red]⚠ errors[/red]" if trace.had_errors else ""
...
def print_traces_list(traces: list[TurnTrace]) -> None:
    if not traces:
        print_info("No traces for today.")
        return
    for trace in traces:
...
def print_stats(stats: SessionStats) -> None:
    ...
    table.add_row("Session", _short_id(stats.session_id))
    table.add_row("Model", stats.model)
    table.add_row("Duration", f"{stats.elapsed_minutes()} min")
    table.add_row("Turns", str(stats.total_turns))
    ...
    _console.print(
        Panel(table, title=f"[bold]Session [{_short_id(stats.session_id)}][/bold]", border_style="blue")
    )


def print_memory_list(results: list) -> None:
    if not results:
        print_info("No facts saved in memory.")
        return
