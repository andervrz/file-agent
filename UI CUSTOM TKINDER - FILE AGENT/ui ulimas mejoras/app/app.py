...
CAMBIO CLAVE (v2.1): NUNCA bloquear el UI thread.
Toda llamada async al backend usa add_done_callback + self.after().
"""
...
class FileAgentApp(ctk.CTk):
    ...
    def _after_load_history(self, session_id: str, messages: list[ChatMessage]) -> None:
        """Paso 3: Aplicar a UI y cargar session metadata."""
