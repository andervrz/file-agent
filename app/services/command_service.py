...
CAMBIO (v2.2): /memory adaptado al nuevo API (profile + procedural + keywords).
Eliminados search_facts/get_recent_facts/clear_all. Ahora usa:
  - get_all_profile() / get_all_procedural_rules() / get_session_keywords()
  - cleanup_expired() para limpieza
"""
...
class CommandService:
    ...
    def _cmd_help(self) -> str:
        return (
            ...
            "  /memory            Show user profile, active rules, recent keywords\n"
            "  /memory profile    Show user profile only\n"
            "  /memory rules      Show procedural rules only\n"
            "  /memory keywords   Show recent session keywords\n"
            "  /memory clear      Delete expired keywords and stale rules\n"
    ...
    async def _cmd_memory(self, args: list[str]) -> str:
        """
        NUEVO (v2.2): /memory adaptado al modelo de 3 capas.
        Soporta: /memory, /memory profile, /memory rules, /memory keywords, /memory clear
        """
        ...
        # /memory clear — limpiar expirados y stale
        if subcmd == "clear":
            kw_deleted = await self._memory_service.cleanup_expired()
            stale = await self._memory_service.cleanup_stale()
            return (
                f"✓ Memory cleanup complete.\n"
                f"  Expired keywords removed: {kw_deleted}\n"
                f"  Stale rules flagged: {stale}"
            )

        # /memory profile — solo perfil
        if subcmd == "profile":
            profile = await self._memory_service._store.get_all_profile()
            if not profile:
        ...
        # /memory rules — solo reglas procedurales
        if subcmd == "rules":
            rules = await self._memory_service._store.get_all_procedural_rules(limit=50)
            if not rules:
                return "📋 No procedural rules stored yet."
            lines = [f"📋 Procedural Rules ({len(rules)}):\n"]
            for i, rule in enumerate(rules, 1):
                status = "🟢" if not rule.provisional else "🟡"
                lines.append(
                    f"{status} {i}. [{rule.trigger}] {rule.action} "
                    f"(hits: {rule.hit_count})"
                )
            return "\n".join(lines)

        # /memory keywords — solo keywords de sesión actual
        if subcmd == "keywords":
            session_id = self._bridge.session_id
            if not session_id:
                return "⚠️ No active session."
            keywords = await self._memory_service._store.get_session_keywords(
                session_id, limit=20
            )
            if not keywords:
                return "🔑 No keywords for this session yet."
            lines = [f"🔑 Session Keywords ({len(keywords)}):\n"]
            for kw in keywords:
                ctx = f" → {kw['context']}" if kw.get("context") else ""
                lines.append(f"  • {kw['keyword']}{ctx} (hits: {kw['hit_count']})")
            return "\n".join(lines)

        # /memory (sin args) — resumen completo
        lines: list[str] = []
        ...
        # Reglas activas
        rules = await self._memory_service._store.get_all_procedural_rules(limit=20)
        active_rules = [r for r in rules if not r.provisional]
        if active_rules:
            lines.append(f"📋 Active Rules ({len(active_rules)}):")
            for rule in active_rules[:5]:
                lines.append(f"  • {rule.trigger}: {rule.action}")
            lines.append("")
        ...
        if session_id:
            ...
            if keywords:
                lines.append(f"🔑 Recent Keywords ({len(keywords)}):")
                for kw in keywords[:5]:
                    ctx = f" → {kw['context']}" if kw.get("context") else ""
                    lines.append(f"  • {kw['keyword']}{ctx}")
                lines.append("")

        if not lines:
            return "🧠 Memory is empty. Start chatting to build profile, rules, and keywords."
