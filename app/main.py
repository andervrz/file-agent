...
CAMBIO (v2.2): Recibe memory_service del bootstrap y lo inyecta en
FileAgentApp y CommandService.
"""
...
def _bootstrap_thread(q: queue.Queue) -> None:
    ...
    try:
        ...
        harness, memory, recorder, config, skills_count, memory_service = loop.run_until_complete(
            bootstrap()
        )
        q.put(("ok", (harness, memory, recorder, config, skills_count, memory_service)))
...
def main() -> None:
    ...
    harness, memory, recorder, config, skills_count, memory_service = payload
    ...
    # NUEVO: cargar working memory cache para la sesión inicial
    if memory_service:
        try:
            asyncio.run(memory_service.load_session(session.session_id))
        except Exception as exc:
            logger.warning("Failed to load memory cache for initial session: %s", exc)
